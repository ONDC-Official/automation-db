...
/**
 * Fetches a specific payload by Payload ID
 */
export const getPayloadByPayloadId = async (req: Request, res: Response) => {
  const { payloadId } = req.params;
  try {
    logger.info(`Fetching payload with ID: ${payloadId}`); // Log the action with ID
    const payload = await payloadService.getPayloadByPayloadId(payloadId);
    if (payload) {
      res.json(payload); // Return the payload details if found
    } else {
      logger.warn(`Payload with ID: ${payloadId} not found`); // Log warning if payload not found
      res.status(404).send("Payload not found"); // Send not found response
    }
  } catch (error) {
    logger.error(`Error retrieving payload with ID: ${payloadId}`, error); // Log error with ID
    res.status(400).send("Error retrieving payload"); // Send bad request response
  }
};
...
export const updatePayload = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updatedPayload = await payloadService.updatePayload(Number(id), payloadData);
    if (updatedPayload) {
...
export const getPayloadByTransactionId = async (
  ...
  try {
    ...
    logger.error(`Error retrieving payload with transactionId: ${transactionId}`, error); // Log error with transactionId
    res.status(400).send("Error retrieving payload"); // Send bad request response
  }
};