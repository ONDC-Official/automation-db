 ...
 */
export const getPayloadById = async (
	req: Request,
	res: Response
): Promise<void> => {
	const { id } = req.params;
 ...
 */
export const getPayloadByPayloadIds = async (
	req: Request,
	res: Response
): Promise<void> => {
...
export const getPayloadsByDomainAndVersion = async (
	req: Request,
	res: Response
) => {
	try {
		...
		logger.error("Error retrieving payloads by domain and version", error);
		res.status(400).send("Error retrieving payloads");
	}
};
